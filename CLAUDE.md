...
- **Cell value == color index == piece type.** A board cell holds `0` or `1..7`; `PIECES[n]` is filled entirely with the literal `n`, and `COLORS[n]` is that piece's color. Both arrays are 1-indexed with a leading `null`. Adding or reordering a piece means editing `PIECES`, `COLORS`, and the `Math.random() * 7` in `randomPiece()` together.
